<template>
  <div class="box">
    <el-row>
      <el-col :span="5">
        <el-input placeholder="请输入内容" v-model="info.query" @input="changeInput" class="input-with-select">
...
import { getUserList, delUser, updateUserAss, setUser, updateUser } from "@/api/user.js"
  ...
  methods: {
    ...
    closeChildDialog(info) {
      ...
      if (info.id == "") {
        ...
        console.log(info)
        updateUser({ uId: info.id, type: info.type }).then((res) => {
          console.log(res)
        })
      }
    },
    ...
    debounce(fn, interval, immediate = false) {
      /**上一次执行的时间*/
      let lastStartTime = 0
      let timer = null
      let context, args
